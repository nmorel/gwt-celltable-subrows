 ...
 */
public abstract class MyAbstractCellTableBuilder<T> implements CellTableBuilder<T> {
   ...
   */
  public MyAbstractCellTableBuilder(AbstractCellTable<T> cellTable) {
   ...
   * @param rowValue the value for the row corresponding to the element
   */
  public final <C> void renderChildCell(ElementBuilderBase<?> builder, Context context,
      HasCell<T, C> column, C value) {
    // Generate a unique ID for the cell.
    String cellId = cellToIdMap.get(column);
    if (cellId == null) {
      cellId = "cell-" + Document.get().createUniqueId();
    ...
    // Render the cell into the builder.
    SafeHtmlBuilder cellBuilder = new SafeHtmlBuilder();
    column.getCell().render(context, value, cellBuilder);
    builder.html(cellBuilder.toSafeHtml());
  }
